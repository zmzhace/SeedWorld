...
export default function NewWorldPage(){
  ...
  async function submit(event:React.FormEvent){event.preventDefault();if(!form.prompt.trim()||creating)return;setCreating(true);setError('');try{setStage('创建作品空间');const createdResponse=await fetch('/api/worlds',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:form.title||undefined,worldPrompt:form.prompt,writingSettings:{genre:form.genre,audience:form.audience,language:'zh',narration:'third_limited',targetWords:form.targetWords,pacing:form.pacing,chapterMode:'automatic'}})});const created=await createdResponse.json();if(!createdResponse.ok)throw new Error(created.error||'创建失败');createLegacyMirror({id:created.id,worldPrompt:form.prompt});if(created.snapshot)localStorage.setItem(`world_${created.id}`,JSON.stringify(created.snapshot));setStage('保存并解析原始资料');const data=new FormData();data.set('text',[form.prompt,form.material].filter(Boolean).join('\n\n'));files.forEach((file)=>data.append('files',file));const sourceResponse=await fetch(`/api/worlds/${created.id}/sources`,{method:'POST',body:data});const source=await sourceResponse.json();if(!sourceResponse.ok)throw new Error(source.error||'资料处理失败');setStage('编译动态本体与世界档案');const extractResponse=await fetch(`/api/worlds/${created.id}/extractions`,{method:'POST'});const job=await extractResponse.json();if(!extractResponse.ok)throw new Error(job.error||'抽取启动失败');router.push(`/worlds/${created.id}?job=${job.id}&step=1`)}catch(cause){setError(cause instanceof Error?cause.message:String(cause));setCreating(false)}}
  return <main className="new-world"><header className="new-world-head"><button onClick={()=>router.push('/')}><ArrowLeft size={15}/>返回首页</button><strong>SEEDWORLD</strong></header><div className="new-world-shell">
    ...
    <form className="new-world-form" onSubmit={submit}><section className="world-materials"><h2 className="form-section-title"><BookOpenText size={17}/>作品底稿</h2>
      ...
      <button className="submit-world" disabled={creating||!form.prompt.trim()}><span>{creating?'正在创建……':'创建并编译世界档案'}</span><ArrowRight size={16}/></button>
