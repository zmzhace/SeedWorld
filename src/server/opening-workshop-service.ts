...
function openingStyleIssues(markdown: string): string[] {
  ...
  const systemIndex = text.search(/系统|面板|绑定|任务|属性|奖励/)
  if (systemIndex >= 0 && systemIndex < Math.floor(text.length * 0.3)) issues.push('系统过早出现，尚未建立现实压迫和主角主动反击')
  if (systemIndex >= 0 && !/(打脸|反击|翻盘|逼退|反制|还手|夺回|拆穿|逼得)[^。！？]{0,100}(?:系统|面板|绑定|任务|属性|奖励)/.test(text)) issues.push('系统首次出现前缺少可见的现实反击，爽点没有递进')
  return issues
}
...
` }], { maxTokens: 2400, maxAttempts: 2 })
    ...
    if (isSystemStory && !allowedConcepts.some((value) => /系统|金手指|外挂|回收|能力|面板/.test(value))) throw new Error('系统题材首章必须保留一个明确但不解释的系统/能力概念')
