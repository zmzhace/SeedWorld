...
export const OUTLINE_RULES = [
  ...
  'Build an explicit chain: what changed before the scene, what the viewpoint character notices, what choice follows, and what consequence closes the scene.',
  'Never jump between unrelated locations, factions, or conflicts without a transition and a causal reason for arrival.',
  'Treat the approved chapter outline as the destination: character agency may determine how it is reached, never replace it with unrelated activity.',
  'Every planned chapter must name its causal prerequisite, its contribution to the confirmed ending, its reader payoff, its reveal boundary, and the exact loss caused by deleting it.',
  'A hook is not a payoff. Deliver one concrete answer, success, reversal, decision, or emotional result before opening the next question.',
].join('\n')
...
export const DRAFT_RULES = [
  ...
  'The world may contain many agents, but the chapter is not a roll call: name only the contracted cast and authorized off-stage references.',
  'Never give each character a status paragraph. Build one continuous action-reaction-consequence chain and let characters enter only when they change it.',
  'Do not distribute page time evenly. Narrative importance, not agent existence, determines who appears and how much space they receive.',
  ...
  '- No unexplained plot jumps: every major action must have a visible trigger, an understandable intention, and an immediate consequence.',
  '- Keep the reader oriented at every scene break: state who is present, where they are, when it is, and what has just changed.',
...
export const VALIDATE_RULES = [
  ...
  '- 群像点名册: mentioning agents one by one merely because they exist in the simulation.',
  '- 并列行动汇报: separate character updates that do not change one shared conflict.',
