...
export function NovelStudioPanel({ worldId, tick, onWorldUpdate }: { worldId:string; tick:number; onWorldUpdate?:(world:any)=>void }) {
  ...
  return <div className="simulation-studio">
    <div className="sim-toolbar">
      <div><h2>下一章</h2><p>系统先确定本章发生什么，再由 Writer 写成完整正文；两者属于同一次生成。</p></div>
      <button className="run-tick" disabled={ticking || locked || !snapshot} onClick={oneTick}>{ticking || pipelineBusy ? <RefreshCw className="spin" size={16}/> : <Activity size={16}/>}<span>{ticking ? '正在启动……' : tickBusy ? workflowLabel(tickRun.stage) : ['queued','running'].includes(chapterRun?.status) ? 'Writer 正在写作……' : '生成下一章'}</span><ArrowRight size={16}/></button>
    </div>

    {!world?.visibilityConfirmed && <div className="sim-lock"><LockKeyhole size={17}/><div><strong>创作尚未解锁</strong><span>请先到设定库确认秘密与角色认知。</span></div></div>}
